...
<asp:Content ID="Content4" ContentPlaceHolderID="ContentPlaceHolderBody" runat="Server">
    <div class="table-responsive">
        <table class="table table-bordered table-condensed table-hover TableSorter">
            ...
            <tbody>
                <asp:Repeater ID="RepeaterAtribut" runat="server" OnItemCommand="RepeaterAtribut_ItemCommand">
                    <ItemTemplate>
                        <tr>
                            <td><%# Container.ItemIndex + 1 %></td>
                            <td><%# Eval("TBAtributGrup.Nama") %></td>
                            <td><%# Eval("Nama") %></td>
                            <td><%# Pengaturan.FormatDataStatus(Eval("Pilihan")) %></td>
                            <td class="fitSize hidden-print">
                                <a class="btn btn-xs btn-primary" href="Pengaturan.aspx?id=<%# Eval("IDAtribut") %>">Ubah</a>
                                <asp:Button ID="ButtonHapus" runat="server" Text="Hapus" CssClass="btn btn-xs btn-danger" CommandName="Hapus" CommandArgument='<%# Eval("IDAtribut") %>' />
                            </td>
                        </tr>
                    </ItemTemplate>
                </asp:Repeater>
